...
| `src/Permissions/AccessPolicy.cs` | `Evaluate(wearer, requester)` liefert `AccessVerdict` mit Begründung; Ohne Regel immer gewährt |
...
| `src/UI/PermissionPanel.cs` | MonoBehaviour unter `BackpackWheel.transform`, baut Zeilen per Code, `Instance`, `HoveredRow`, `ShowFor`/`HideIfOpen` |
